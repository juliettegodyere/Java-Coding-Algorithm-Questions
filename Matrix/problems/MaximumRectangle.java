...
//Soultion: https://www.geeksforgeeks.org/maximum-size-rectangle-binary-sub-matrix-1s/
class MaximumRectangle {
    ...
    static int maxRectangle(int R, int C, int M[][]) {
        int result = maxHist(R, C, M[0]);
        for (int i = 1; i < R; i++) {
            for (int j = 0; j < C; j++){
                if (M[i][j] == 1){
                    M[i][j] += M[i - 1][j];
            ...
            result = Math.max(result, maxHist(R, C, M[i]));
        }
